...
def get_closest_flame(position: Position, flames: list[Flame]) -> Flame:
    closest_flame = min(flames, key=lambda f: euclidean_distance(position, f.get("position", {})))
    return closest_flame

def sort_flames_by_distance(player_position: Position, flames: list[Flame]) -> list[Flame]:
    return sorted(flames, key=lambda f: euclidean_distance(player_position, f.get("position", {})))
...
def get_closest_bullet_to_flame(flame: Flame, bullets: list[Bullet]) -> Bullet | None:
    if not bullets:
        return None
    closest_bullet = min(bullets, key=lambda b: euclidean_distance(flame.get("position", {}), b.get("position", {})))
