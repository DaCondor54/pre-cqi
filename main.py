...
def get_flame_target(turn_data: GameState) -> Flame:
    ...
    if(euclidean_distance(player_position, closest_flames[0].position) < safe_distance_threshold):
        ...
        return get_closest_flame(player_position, closest_flames, with_campfire=True, bullets=turn_data.bullets)
    return get_closest_campfire(closest_flames) or get_closest_flame(player_position, closest_flames, with_campfire=True, bullets=turn_data.bullets) or get_default_target(turn_data)
...
def get_closest_flame(position: Position, flames: list[Flame], bullets: list[Bullet], with_campfire: bool) -> Flame:
    ...
    for flame in closest_flames:
        if (not is_flame_dying_soon(flame, bullets) and (with_campfire or not going_towards_player(flame, position))):
            return flame
